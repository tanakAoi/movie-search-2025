...
export const RegionSettings = ({ setIsSettingsOpen }: RegionSettingsProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mt-4 text-sm border-t border-base-fg/20 pt-4">
        <div className="flex items-center gap-4">
          {selectedCountry ? (
            <div className="flex items-center gap-2">
              <Image
                src={`https://flagcdn.com/${selectedCountry.iso_3166_1.toLowerCase()}.svg`}
                width={24}
                height={24}
              ...
              />
              <span>
                {selectedCountry.native_name || selectedCountry.english_name}
              </span>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <Image
                src={`https://flagcdn.com/${userData?.country?.iso_3166_1.toLowerCase()}.svg`}
                width={24}
                height={24}
                alt={userData?.country?.english_name || "Unknown"}
                className="rounded-full w-6 h-6 object-cover"
              />
              <span>
          ...
          {selectedLanguage ? (
            <div className="flex items-center gap-2 ml-4">
              <span className="bg-accent-bg text-base-bg px-2 py-1 rounded text-xs font-bold">
                {selectedLanguage.iso_639_1.toUpperCase()}
              </span>
              <span>
            ...
            <div className="flex items-center gap-2 ml-4">
              <span className="bg-accent-bg text-base-bg px-2 py-1 rounded text-xs font-bold">
                {userData?.language?.iso_639_1.toUpperCase()}
              </span>
              <span>
